...
import FollowUpFilter from "../../components/follow-up/FollowUpFilter" // Import AuthContext
...
function FollowUp() {
  ...
  const renderPendingRow = (followUp, index) => (
    <tr key={`${followUp.leadId}-${index}`} className="hover:bg-slate-50 transition-colors group">
      ...
      {visibleColumnsPending.nextCallDate && <td className="px-3 sm:px-4 py-3 sm:py-4 text-sm text-gray-500 whitespace-nowrap">{followUp.nextCallDate || followUp.timestamp}</td>}
